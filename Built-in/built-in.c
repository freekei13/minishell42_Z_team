...
/*   Updated: 2026/08/01 02:12:13 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	if_echo(t_ast *cmd, t_exec *exc_data)
{
	int	i;

	i = 0;
	while (cmd->u_data.cmd.args[1 + i]
		&& echo_opt(cmd->u_data.cmd.args[1 + i]))
		i++;
	if (i > 0)
		echo_fct(cmd->u_data.cmd.args + 1 + i, 1, exc_data);
